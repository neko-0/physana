...
lumi = 138.861
#configMgr.out_path = pathlib.Path('/gpfs/slac/atlas/fs1/u/mgignac/analysis/collinearw/dev/collinearw/configs/main/v5/figures/')
signal = configMgr.get_process_set("wjets_2211")
unfoldeds = [
    procset for procset in configMgr.process_sets if procset.process_type == 'unfolded' and procset.name=='unfold_realthang_fake-MU'
]

eles = [
    procset for procset in configMgr.process_sets if procset.process_type == 'unfolded' and procset.name=='unfold_realthang_fake-EL'
]
...
for region in regions:
  for observable in signal.nominal.get_region(f'electron_{region}_truth'):
      ...
      observable_el = unfolding.plot.scale_to_xsec(
          signal.nominal.get_region(f'electron_{region}_truth').get_histogram(observable.name),
          lumi,
      )

      data_ele = unfolding.plot.scale_to_xsec(
          ele_data.nominal.get_region(f'electron_{region}_truth').get_histogram(observable.name),
          lumi,
      )

      observable_mu = unfolding.plot.scale_to_xsec(
          signal.nominal.get_region(f'muon_{region}_truth').get_histogram(observable.name),
          lumi,
      )

      data_mu = unfolding.plot.scale_to_xsec(
          mu_data.nominal.get_region(f'muon_{region}_truth').get_histogram(observable.name),
          lumi,
      )
