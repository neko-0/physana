import numpy
from collinearw import Histogram, Region
...
def test_region_add_operation():
    ...
    assert (result_region.get_histogram("jetPt").bin_content == result_bin).all()
    assert (result_region.get_histogram("lepPt").bin_content == rbin_content).all()
    ...
    assert (c_lhs_region.get_histogram("jetPt").bin_content == result_bin).all()
    assert (c_lhs_region.get_histogram("lepPt").bin_content == rbin_content).all()
...
def test_region_sub_operation():
    ...
    lbin_content = numpy.array([0, 1, 5, 7, 0])
    # lsumW2 = numpy.array([2, 4, 1])

    rbin_content = numpy.array([0, 2, 4, 3, 0])
    # rsumW2 = numpy.array([3, 1, 2])
