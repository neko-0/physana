...
def run(
    ...
    Args:
        configMgr (physana.configMgr.ConfigMgr): configuration object with all metadata and histograms needed
        measured (physana.core.Process): process we want to measure (used for hMeas)
        truth (physana.core.Process): process we want to unfold on to (used for hMeas, hTrue). This is used a prior in the Bayesian interation.
        response (physana.core.Process): process to use for the response matrix (used for hResponse)
        ...
        additionalTruths (list of physana.core.Process): additional truth processes to overlay
                ...
                if n_unfolds == 1 or exact_niter:
                    ...
                    if saveYoda:
                        ...
                        yoda_data['data'].append(
                            utils.root2yoda(
                                hReco_scaled,
                                path=f"/REF/physana/{unfold_name}_{observable}",
                            )
                        )
                        ...
                        yoda_data[truth.name].append(
                            utils.root2yoda(
                                hTrue_scaled,
                                path=f"/physana/{unfold_name}_{observable}",
                            )
                        )
                        for hist, process in zip(
                            ...
                            yoda_data[process.name].append(
                                utils.root2yoda(
                                    hist, path=f"/physana/{unfold_name}_{observable}"
                                )
                            )
