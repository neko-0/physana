...
from ...core import SystematicBase, SystematicBand, Histogram
...
def compute_quadrature_sum(
    ...
    Args:
        ...
        store_band : bool, default = True
            Create SystematicBand object and store it into the nominal process,
    ...
    Return:
        dictionary of SystematicBand object.
    ...
    for region in nominal.regions:
        for obs in region.histograms:
            ...
            band = SystematicBand(name, syst_type, obs.bin_content.shape)
            lookup_key = (region.name, obs.name)
            for side in ["up", "down"]:
                if components[side]:
                    sub_band = SystematicBand(
...
def create_lumi_band(histogram, lumiunc=0.0, content_type="event"):
    ...
    if syst_band and "lumi" in histogram.systematic_band:
        ...
        lumi_band = SystematicBand("lumi", content_type, histogram.bin_content.shape)
